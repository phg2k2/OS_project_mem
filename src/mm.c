...
{                                                   
  ...
  // Cập nhật vùng ánh xạ trả về
  ret_rg->rg_start = addr;
  ret_rg->rg_end = addr + pgnum * PAGING_PAGESZ;
  ret_rg->vmaid = 0; // mặc định vmaid = 0, hoặc lấy từ caller nếu cần

  // Ánh xạ từng frame vào bảng trang
...
{
  int pgit, fpn;
  struct framephy_struct *head = NULL, *prev = NULL;
  ...
  for (pgit = 0; pgit < req_pgnum; pgit++)
  {
    ...
    {
      // Không đủ frame, giải phóng các frame đã cấp phát
...
/* Sửa lại init_vm_rg để nhận vmaid */
